...
# timm-free patch — stubs locaux pour éviter la dep timm
import collections.abc as _collections_abc
def to_2tuple(x):
    if isinstance(x, _collections_abc.Iterable) and not isinstance(x, str):
        return tuple(x)
    return (x, x)
...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    # PyTorch >=1.8 fournit ça nativement
    return nn.init.trunc_normal_(tensor, mean=mean, std=std, a=a, b=b)
def register_model(fn):
    return fn  # no-op : on appelle les factories directement
